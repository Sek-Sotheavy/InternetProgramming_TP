...
router.post('/create', auth.ensureSignedIn, async (req, res,) => {
  ...
  const result = await priceServiceService.create({
    title,
    price,
    category,
    item,
...
router.post('/update/:id', auth.ensureSignedIn, async (req, res, next) => {
  // to do
  const { title, category, item, user, price, imageUrl, desc, } = req.body;
  const { id } = req.params;
  const result = await priceService.update(id, {
    title,
    category,
    item,
    user,
...
router.post('/delete/:id', auth.ensureSignedIn, async (req, res, next) => {
  // to do
  const { id } = req.params;
  const result = await priceServiceService.remove(id)
  res.json(result);
})
